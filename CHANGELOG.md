...
- CI rejects a pull request title that would make a bad commit subject on `main` — non-English (Korean/CJK) or not Conventional Commits (#19)
...
- `jarvis do` draft PRs use a fixed subject (`chore(jarvis): draft for #N`) instead of embedding the issue title, so the text that lands on the target repo's `main` after a squash merge is not external input; the issue title moved to the PR body (#19)
